...
class ExperimentInfo(BaseExperimentInfo):
    ...
    def __init__(self, config, xpy_exp, subject_id, subject_name):
        ...
        self._left_resp_text = None
        self._right_resp_text = None
    ...
    def response_hotspots(self):
     ...
     # ---------------------------------------------------------------
    @property
    def left_resp_text(self):
        """
        The target text stimuli
        :type: trajtracker.stimuli.MultiTextBox 
        """
        return self._left_resp_text

    @left_resp_text.setter
    def left_resp_text(self, target):
        if self._left_resp_text is not None:
            raise ttrk.InvalidStateError("ExperimentInfo.left_resp_text cannot be set twice")

        self._left_resp_text = target
        self.stimuli.add(target.stimulus, "left_resp_text")

    @left_resp_text.getter
    def get_left_resp_text(self):
        return self._left_resp_text

    # ---------------------------------------------------------------
    @property
    def right_resp_text(self):
        """
        The target text stimuli
        :type: trajtracker.stimuli.MultiTextBox 
        """
        return self._right_resp_text

    @right_resp_text.setter
    def right_resp_text(self, target):
        if self._right_resp_text is not None:
            raise ttrk.InvalidStateError("ExperimentInfo.right_resp_text cannot be set twice")

        self._right_resp_text = target
        self.stimuli.add(target.stimulus, "right_resp_text")

    @right_resp_text.getter
    def get_right_resp_text(self):
        return self._right_resp_text
