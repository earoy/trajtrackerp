...
def create_response_buttons(exp_info):
    ...
    if texts is None:
        ...
        common.validate_config_param_type("resp_btn_font_size", int, config.resp_btn_font_size)
        ...
        for i in range(2):
            ...
            ## Block that allows for multiple text boxes with different color fonts
            ## for IAT type tasks. Colors are hardcoded for now

            if exp_info.config.multiple_response:

                print(config.text_target_colour)

                button1 = xpy.stimuli.TextBox(text='texts[i]',
                                         size=size,
                ...
                exp_info.response_buttons.append(button1)
                exp_info.stimuli.add(button1, visible=True, stimulus_id="resp_btn1_%d" % i)

                button2 = xpy.stimuli.TextBox(text='t2',
                                             size=size,
                                             position=tuple(map(operator.add,positions[i],(0,-50))),
                                             text_font=config.resp_btn_font_name,
                                             text_size=config.resp_btn_font_size,
                                             text_colour=xpy.misc.constants.C_GREEN,
                                             background_colour=colors[i])

                exp_info.response_buttons.append(button2)
                exp_info.stimuli.add(button2, visible=True, stimulus_id="resp_btn2_%d" % i)

            #regular discrete choice paradigm
            else:
                button1 = xpy.stimuli.TextBox(text=texts[i],
                                         size=size,
                                         position=positions[i],
    ...
    for i in range(2):
        if exp_info.config.multiple_response:
            area = ttrk.misc.nvshapes.Rectangle(size=size, position=tuple(map(operator.add,positions[i],(0,-50))))
...
def _get_feedback_rect_sizes(exp_info):
    ...
    if size_param is not None and not isinstance(size_param, numbers.Number):
        ...
        try:
            ...
            single_size = common.xy_to_pixels(size_param, exp_info.screen_size, 'config.feedback_rect_size')
        ...
        result = [common.xy_to_pixels(s, exp_info.screen_size, 'config.feedback_rect_size') for s in size_param]
...
def load_data_source(exp_info):
    ...
    loader.add_field('resp_btn_texts', str, optional=True)
